...
export default function ComentariosProductos() {
    ...
    React.useEffect(() => {
      axios.get("http://localhost/proyectoTiendas/filtrarComentariosProductos.php?filtro1=0&filtro2=&inicio=&fin=")
        .then(response=>{
     ...
     const getData=(f1, f2, ini, fin)=>{
        ...
        axios.get("http://localhost/proyectoTiendas/filtrarComentariosProductos.php?filtro1="+f1+"&filtro2="+f2+"&inicio="+ini+"&fin="+fin)
        .then(response=>{
          setRow(response.data);
          console.log(response.data);
        }).catch(error=>{
  ...
  return (
    <TableContainer component={Paper} sx={{justifyContent: "center" }} >
      <h3>Comentarios y Calificaciones Productos</h3>
      <p>Ordenar por:</p>
      ...
      <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table" id="tabla-comentarios-productos">
      <TableHead>
          <TableRow>
            ...
            >
              ...
              <TableCell align="right">{ro.fecha}</TableCell>
                ...
                <ReactHTMLTableToExcel
                    ...
                    table="tabla-comentarios-productos"
                    filename="tabla-comentarios-productos"
                    sheet="comentarios_productos"
