...
$method = $_SERVER['REQUEST_METHOD'];
    ...
    if($busqueda!=''){
      $sql = "select * from (select p.id_producto id, getLink(p.id_producto, 0) link, GROUP_CONCAT(c.nombre_categoria SEPARATOR ', ') categorias FROM categoria_producto p inner join categorias c where p.id_categoria = c.id group by id_producto) l inner join productos_aprobados on l.id = productos_aprobados.id where nombre like '%$busqueda%' or precio like '%$busqueda%' or descripcion like '%$busqueda%' or categorias like '%$busqueda%' or fecha like '%$busqueda%';";
    }else{
